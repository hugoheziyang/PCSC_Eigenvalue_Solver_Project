...
TEST(SolveShiftedLinearSystem, ThrowsOnNonSquareDense) {
    ...
    Matrix A_wrapped(A);
    ShiftedOptions<Scalar> opts(Scalar(1.0));

    Vec b(2);
    b.setOnes();

    EXPECT_THROW(
        (void)solve_shifted<Scalar>(A_wrapped, opts, b),
        std::runtime_error
    );
}
...
TEST(SolveShiftedLinearSystem, ThrowsOnNonSquareSparse) {
    ...
    Matrix A_wrapped(A);
    ShiftedOptions<Scalar> opts(Scalar(0.5));
...
TEST(SolveShiftedLinearSystem, ThrowsOnScalarTypeMismatch) {
    ...
    ShiftedOptions<ComplexScalar> opts(ComplexScalar(1.0, 0.0));
    ...
    EXPECT_THROW(
        (void)solve_shifted<ComplexScalar>(A_wrapped, opts, b),
        std::runtime_error
    );
}
